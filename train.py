...
def _train(model, args):
    ...
    for epoch_idx in range(epoch, args.num_epoch + 1):
        ...
        for iter_idx, batch_data in enumerate(dataloader):
            ...
            label_fft1 = torch.view_as_real(torch.fft.rfft(label_img4, dim=2, norm=None))
            pred_fft1 = torch.view_as_real(torch.fft.rfft(pred_img[0], dim=2, norm=None))
            label_fft2 = torch.view_as_real(torch.fft.rfft(label_img2, dim=2, norm=None))
            pred_fft2 = torch.view_as_real(torch.fft.rfft(pred_img[1], dim=2, norm=None))
            label_fft3 = torch.view_as_real(torch.fft.rfft(label_img, dim=2, norm=None))
            pred_fft3 = torch.view_as_real(torch.fft.rfft(pred_img[2], dim=2, norm=None))
